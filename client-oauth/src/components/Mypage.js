...
class Mypage extends Component {
  ...
  async getGitHubUserInfo() {
    ...
    const res = await axios.get('https://api.github.com/user', { headers: { authorization : `token ${this.props.accessToken}` } })
    this.setState({
      name: res.data.name,
    ...
    // await octokit.request('GET https://api.github.com/user')
    // .then(res=>{
    //   console.log(res);
    // })
    ...
    return (
      <div>
        <div className='mypageContainer'>
          ...
          <div id="images">
            {
            this.state.images.map((image, index) => <img key={index} src={image.blob} alt={image.file} />)
